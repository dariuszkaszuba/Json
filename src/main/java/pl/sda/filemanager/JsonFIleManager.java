...
public class JsonFIleManager implements FileManager {
    ...
    private FilesMetaData filesMetaData; // = new FilesMetaData();
    private ObjectMapper objectMapper; //= new ObjectMapper();


    public JsonFIleManager(Path path) {
    ...
    public void addTag(String tag, Path path) throws FileManagerException {
        ...
        try {
            ...
            throw new FileManagerException(String.format("Failed to add tag %s to %s", tag, metaDataFile), e);
        }
    }

    @Override
    public List<Path> findFilesByTag(String tag) throws FileManagerException {
//        return filesMetaData.getEntries().stream().filter(fileEntry -> fileEntry.getTags().contains(tag)).map(fileEntry -> Paths.get(fileEntry.getPath())).collect(Collectors.toList());
        // lub foreachem =D
